...
ax.plot(theory_Rnorm, theory_mfpPerp/Lc_slab, '--', color='red', lw=3, alpha=.5)
...
ax.set_ylabel('$\lambda_{\perp} / L_c^{slab}$', fontsize=20.)
...
ax.plot(adim_RLs, adim_MFPs_perp, '-o', c='blue', label='$L_c^{2D} / L_c^{slab} = 1$')
ax.plot(theory_Rnorm, theory_mfpPerp/Lc_slab, '--', c='blue', lw=3, alpha=.5)
TITLE1  = '$L_c^{2D}[AU]: %1.1e$   $L_c^{slab}[AU]: %1.1e$' % (Lc_2d, Lc_slab)
TITLE0  = '$Nm:%d$    $\sigma_{slab}^2/\sigma^2$: $%1.2f$    $(\sigma/Bo)^2$: $%1.1e$' %  (Nm, perc_slab, sig)
TITLE2 	= '$L_c^{2D} / L_c^{slab} = 0.1$'
TITLE   = TITLE0 + '\n' + TITLE2 + '\n'
...
fname_fig       = '%s/mfpPerp.vs.Rnorm_two.cases_sig.%1.2f_Nm%03d_slab.%1.2f.png' % (dir_figs, sig, Nm, perc_slab)
